...
$getplace = $_GET["place"];
 ...
 if (isset($getgender)){
 $result_msg = mysql_query("SELECT * FROM duitasuomsg WHERE gender='$getgender' ORDER BY timesubmit DESC;") or die();
 } else if (isset($gettime)){
 $result_msg = mysql_query("SELECT * FROM duitasuomsg WHERE time='$gettime' ORDER BY timesubmit DESC;") or die();
 } else if (isset($getplace)){
 $result_msg = mysql_query("SELECT * FROM duitasuomsg WHERE place='$getplace' ORDER BY timesubmit DESC;") or die();
 } else {
 $result_msg = mysql_query("SELECT * FROM duitasuomsg ORDER BY timesubmit DESC;") or die();
 }
 */
 
 $result_msg = mysql_query("SELECT * FROM coursemsg ORDER BY timesubmit DESC;") or die();
 ...
 $msgcourseid = $row_msg['courseid'];
 
 if ($msgowner == 1 && $msgcourseid == $courseid){
 $msgid = $row_msg['id'];
 $role = $row_msg['role'];
...
if ($imagelocation != ""){
 ...
 echo "<a><li class='post_action' id='edit_msg_".$msgid."' onclick='edit_msg(".$msgid.")' onmouseover=this.className='post_action_onmouseover' onmouseout=this.className='post_action'>edit</li></a>
       <a><li class='post_action' id='cancel_msg_".$msgid."' onclick='cancel_msg_my_msgs(".$msgid.")' onmouseover=this.className='post_action_onmouseover' onmouseout=this.className='post_action'>delete</li></a>";
...
if ($mymsgs == 0){
echo "<div class='post'>
      <h4 onselectstart='return false'>You did not post anything in this class.</h4>
      </div>";
}
