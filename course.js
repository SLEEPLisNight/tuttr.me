...
function refresh_coursemsg(pn,courseid)
{
...
xmlhttp_refresh_duitasuo.open("GET","server_refresh_coursemsg.php?pn="+pn+"&courseid="+courseid,true);
...
xmlhttp_edit_msg.open("GET","server_edit_coursemsg.php?id="+id,true);
...
function cancel_msg(id,courseid) 
{
    ...
    "<div class=\"buttons\">"+
    "<p class=\"pmsg_close_question_yes\" onclick=\"cancel_msg_confirm("+id+","+courseid+")\">Yes</p>"+
...
function cancel_msg_confirm(id,courseid)
{
if (window.XMLHttpRequest)
  {// code for IE7+, Firefox, Chrome, Opera, Safari
...
xmlhttp_cancel_msg.onreadystatechange=function()
  ...
  if (xmlhttp_cancel_msg.readyState==4 && xmlhttp_cancel_msg.status==200)
	...
	refresh_coursemsg('0',courseid);
    }
  }
xmlhttp_cancel_msg.open("GET","server_delete_coursemsg.php?id="+id,true);
...
xmlhttp_cancel_msg_unread_comment.open("GET","server_delete_coursemsg.php?id="+id,true);
...
xmlhttp_unread_comment.open("GET","server_unread_coursecomment.php", true);
...
xmlhttp_unread_comment_sendemail.open("GET","server_unread_sendemail_course.php?update="+update,true);
...
xmlhttp_see_unread_comment.open("GET","server_see_unread_coursecomment.php",true);
...
function course_login(schoolid,courseid)
{
var renrenuserid = 0;
...
xmlhttp_login.onreadystatechange=function()
  ...
  if (xmlhttp_login.readyState==4 && xmlhttp_login.status==200)
    ...
    if (xmlhttp_login.responseText=="login"){
		window.location="course.php?schoolid="+schoolid+"&courseid="+courseid+"";
    } else {
...
function see_my_coursemsgs()
{
document.getElementById("duitasuo_display").innerHTML = 
"<div id=\"post_status\" onselectstart=\"return false\">"+
"<div id=\"post_status_control\" class=\"post_status_control\">"+
"<a id=\"refresh_posts\" class=\"refresh_posts\" onclick=\"see_my_coursemsgs()\"><img src=\"images/loading.gif\"></a>"+
"</div></div>"+
"<div id=\"posts\" class=\"posts\"></div>";
...
xmlhttp_see_my_msgs.open("GET","server_see_my_coursemsgs.php",true);
xmlhttp_see_my_msgs.send();
}

function post_coursemsg(schoolid, courseid){
	...
	if (document.getElementById("words").value =="" || document.getElementById("words").value == "and I have a question..."){
		...
		if (document.getElementById("image_preview").style.display != "none"){
			...
			document.getElementById("uploadimage_courseid").value = courseid;
			...
			xmlhttp_post_msg.open("GET","server_post_coursemsg.php?role="+role+"&words="+words+"&imagelocation="+imagelocation+"&courseid="+courseid,true);
...
function course_search(university, courseid){
...
xmlhttp_campus_search.open("GET","server_course_search_course.php?name="+coursename+"&university="+university+"&original_courseid="+courseid,true);
xmlhttp_campus_search.send();

}

function update_usercourses(courseid, schoolid, original_courseid){
	if (window.XMLHttpRequest)
	  {// code for IE7+, Firefox, Chrome, Opera, Safari
	  xmlhttp_campus_search=new XMLHttpRequest();
	  }
	...
	xmlhttp_campus_search.onreadystatechange=function()
	  ...
	  if (xmlhttp_campus_search.readyState==4 && xmlhttp_campus_search.status==200)
		{
		window.location="course.php?schoolid="+schoolid+"&courseid="+original_courseid+"";
		}
	  }
...
function see_course_users(role, courseid){
	if (role == 1){
		document.getElementById("course_students").style.color = "#444";
		document.getElementById("course_tutors").style.color = "#B2B2B2";
	} else if (role == 2){
		document.getElementById("course_students").style.color = "#B2B2B2";
		document.getElementById("course_tutors").style.color = "#444";
	}
	
	if (window.XMLHttpRequest)
	  {// code for IE7+, Firefox, Chrome, Opera, Safari
	  xmlhttp_see_course_users=new XMLHttpRequest();
	  }
	else
	  {// code for IE6, IE5
	  xmlhttp_see_course_users=new ActiveXObject("Microsoft.XMLHTTP");
	  }
	xmlhttp_see_course_users.onreadystatechange=function()
	  {
	  if (xmlhttp_see_course_users.readyState==4 && xmlhttp_see_course_users.status==200)
		{
		document.getElementById("course_students_tutors").innerHTML = xmlhttp_see_course_users.responseText;
		}
	  }
	xmlhttp_see_course_users.open("GET","server_see_course_users.php?role="+role+"&courseid="+courseid,true);
	xmlhttp_see_course_users.send();
}
...
//$("#message").empty();
	...
	$.ajax({
		url: "server_upload_picture_coursemsgs.php", // Url to which the request is send
		...
		success: function(data)   // A function to be called if request succeeds
		{
			...
			var courseid = document.getElementById("uploadimage_courseid").value;
			...
			if (data.indexOf("Error") > -1){ //check if data contain "Error"
				...
				xmlhttp_post_msg.open("GET","server_post_coursemsg.php?role="+role+"&words="+words+"&imagelocation="+data+"&courseid="+courseid,true);
				xmlhttp_post_msg.send();
			}
		}
	});
...
function deletecourse(delete_courseid, schoolid, courseid){
	if (window.XMLHttpRequest)
	  {// code for IE7+, Firefox, Chrome, Opera, Safari
	  xmlhttp_campus_search=new XMLHttpRequest();
	  }
	...
	xmlhttp_campus_search.open("GET","server_delete_usercourses.php?courseid="+delete_courseid,true);
	xmlhttp_campus_search.send();
}
