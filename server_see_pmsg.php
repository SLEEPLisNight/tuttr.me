...
if ($lastPage != "1"){
    ...
    */
    $paginationDisplay .= '<a class="pmsg_pages_nav" onclick="see_pmsg_page(1)">First</a>';
    // If we are not on page 1 we can place the Back button
    if ($pn != 1) {
        $previous = $pn - 1;
        $paginationDisplay .= '<a class="pmsg_pages_nav" onclick="see_pmsg_page('.$previous.')">Previous</a>';
    } 
    ...
    if ($pn != $lastPage) {
        $nextPage = $pn + 1;
        $paginationDisplay .= '<a class="pmsg_pages_nav" onclick="see_pmsg_page('.$nextPage.')">Next</a>';
    } 
    $paginationDisplay .= '<a class="pmsg_pages_nav" onclick="see_pmsg_page('.$lastPage.')">Last</a>';
}
...
echo "<div class='pmsg_pages'>
      <div class='pmsg_pages_total_number'>Total: ".$pmsgs_num."</div>
...
while ($row = mysql_fetch_assoc($duitasuo_pmsg))
{
 ...
 $sendtime = date('Y-m-d g:ia',strtotime($timezonename,$timesubmit));
 ...
 $replytime = date('Y-m-d g:ia',strtotime($timezonename,$timesubmit_reply));
 
 if ($pmsg_reply_receiver == 1 && $readit_reply == 0){
 echo "<tr id='pmsg_".$pmsgid."' class='unread_pmsg'>";
 echo "<td class='name_and_date'><span class='fromusername'><a href='profile.php?userid=".$fromuserid_reply."'>".$fromusername_reply."</a></span>
           <span class='date'>".$replytime."</span></td>";
 echo "<td class='title_and_content' onclick=see_pmsg_content(".$pmsgid.",\"1\")><a><span class='title'>Reply: ".$title."</span></a>
           <a><span class='words'>".$content_reply."</span></a></td>";
 } else {
 echo "<tr id='pmsg_".$pmsgid."' class='read_pmsg'>";
 echo "<td class='name_and_date'><span class='fromusername'><a href='profile.php?userid=".$fromuserid_reply."'>".$fromusername_reply."</a></span>
           <span class='date'>".$replytime."</span></td>";
 echo "<td class='title_and_content' onclick=see_pmsg_content(".$pmsgid.",\"0\")><a><span class='title'>Reply: ".$title."</span></a>
           <a><span class='words'>".$content_reply."</span></a></td>";
 }
 echo "</tr>";
 ...
 echo "<tr id='pmsg_".$pmsgid."' class='unread_pmsg'>";
 echo "<td class='name_and_date'><span class='fromusername'><a href='profile.php?userid=".$fromuserid."'>".$fromusername."</a></span>
...
if ($pmsgs_num == 0){
echo "<div class='no_pmsgs'>No Messages</div>";
} else if($pmsgs_num > 0) {
