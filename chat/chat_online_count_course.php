...
foreach($users as $key => $data) {
	...
	if(time() - $lastvisit[3] >= $sessionTime) {
		...
		if($lastvisit[0] == $userid && $lastvisit[1] == $userchatid && $lastvisit[2] == $courseid) {
			$alreadyIn = TRUE;
			$users[$x] = "$userid|$userchatid|$courseid|" . time(); //updating
		}
	}
	$x++;
}
...
if($alreadyIn == FALSE) {
	$users[] = "$userid|$userchatid|$courseid|" . time();
}
...
mysql_query("INSERT INTO coursechatname (userid,userchatid,username,courseid,loginas,place,xpos,ypos) VALUES('$userid','$userchatid','$username','$courseid','0','','0','0')") or die(mysql_error());
}

$result_chatname_delete = mysql_query("SELECT * FROM coursechatname ORDER BY id ASC;") or die();
...
foreach($users as $key2 => $data) {
	...
	if ($row_chatname_delete['userchatid'] == $onlinechatid && $row_chatname_delete['userid'] == $onlineid && $row_chatname_delete['courseid'] == $onlinecourseid)
	{
	$chatname_delete = 0;
	}
...
mysql_query("DELETE FROM coursechatname WHERE id = '$chatname_delete_id'") or die(mysql_error());
break;
}
