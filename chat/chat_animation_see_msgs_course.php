...
$otherid = $_GET['otherid'];
 ...
 $timenow = time();
 $msgs = 0;
 $result_msg = mysql_query("SELECT * FROM coursemsg WHERE userid = '$otherid' ORDER BY timesubmit DESC;") or die();
 while($row_msg = mysql_fetch_assoc($result_msg)){
 $othercourseid = $row_msg['courseid'];	
 $schools_result = mysql_query("SELECT * FROM classes WHERE id = '$othercourseid' ORDER BY id ASC;") or die();
 $schools_row = mysql_fetch_array($schools_result);
 $otherschoolname = $schools_row['coursenumber'];	
 ...
 if ($imagelocation != ""){
	 ...
	 echo "<div class='post'>
		   ...
		   </a> (".$role.") posted on ".$otherschoolname.": </h4>
