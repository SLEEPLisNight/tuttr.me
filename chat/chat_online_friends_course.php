...
foreach($users as $key => $data) {
	...
	if($onlinechatid != $userchatid && $onlinecourseid == $courseid){
		...
		while($row = mysql_fetch_array($result)){
			...
			$result_usercourses = mysql_query("SELECT * FROM usercourses WHERE userid='$onlineid' AND courseid='$courseid' ORDER BY id ASC;") or die();
  ...
  if ($schoolmate=="")
        {
        $schoolmate="<div class='chat_online_user' onmouseover=this.className='chat_online_user_onmouseover' onmouseout=this.className='chat_online_user' onclick=chatwith('$schoolmatenum[$i]','$schoolmateid[$i]','$schoolmatechatid[$i]','$schoolmatename_encode[$i]')><div class='chat_online_user_name'>".$schoolmatename[$i]."</div><div class='chat_online_user_place'>(".$classmate_role[$i].")</div></div>";
        }
      else
        {
